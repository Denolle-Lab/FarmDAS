...
def sample_walkers(nsamples,flattened_chain):
    ...
    for i in thetas:
        mod = model_dv_temp(i, all=False, **modelparam)
        models.append(mod)
...
if __name__ == '__main__':
    ...
    # model case
    modelparam["modelcase"] = "dv_temp"

    # MCMC parameters
    modelparam["nwalkers"] = 32 # 200 # number of chains

    output_imgdir = "../figure/Fitsoil/MCMC_dv_temp"
    output_datadir = "../processed_fitsoil_dv_temp/MCMC_sampler_{}_Normalized".format(nsteps)
    ...
    usecols=["utvec", "dv", "err", "temp", "soil", "date"]
    root = "../../PREP_data/"
    ...
    #---Read dverature and precipitation data at Parkfield---# 
    # Synchronize the long-period dverature and precipitation
    # store time history of trimmed precipitation and dverature

    modelparam["CAVG"]=(np.array(fi['temp'])-np.mean(np.array(fi['temp'])))
    modelparam["CAVG"]=modelparam["CAVG"]/np.max(np.abs(modelparam["CAVG"]))
    modelparam["dv"]   = np.array(fi['dv'])/np.max(np.abs(fi['dv']))
    ...
    for stationpair in tqdm([stationpairs[i] for i in stationids]):
        ...
        if os.path.exists(foname):
            ...
            # continue;
        dvv_data = np.array(fi["dv"])/np.max(np.abs(fi["dv"]))
        ...
        labels = ["a0","p4","p2","t_{shiftdays}","b_{lin}", "log(f)"]
        ...
        plt.savefig(output_imgdir+"/MCMC_%s_%s_%s_corner.png"%(stnm, freqband, modelparam["modelcase"]), format="png", dpi=100)
        plt.close()
        plt.clf()
        

        med_model, spread = sample_walkers(nsteps,samples)
        best_fit_model = model_dv_temp(theta_max, all=False, **modelparam)
        ...
        for theta in samples[np.random.randint(len(samples),size=9000)]:
            ax[0].plot(uniform_tvec, model_dv_temp(theta, all=False, **modelparam), color="r", alpha=0.1)
        ax[0].plot(uniform_tvec,best_fit_model, c='b',linewidth=0.8, label='Highest Likelihood Model')
        ax[0].plot(uniform_tvec, modelparam["soil"], label='soil-EWT',c='green')
        ax[0].set_title(stnm+" --> theta_max: "+str(title))
        ax[1].plot(uniform_tvec, modelparam["soil"], label='soil-EWT',c='green')
        ...
        ax[2].plot(uniform_tvec, modelparam["soil"], label='soil-EWT',c='green')
        ax[2].plot(uniform_tvec, best_fit_model, c='b',linestyle='--', label='Highest Likelihood Model')
        ax[2].plot(uniform_tvec, modelparam["soil"]-best_fit_model, label='Residual',c='r')
        
        ax[3].plot(uniform_tvec, modelparam["CAVG"],  ls="-", c="orange",label="Normalized de-mean Temp from PRISM")
        ax[3].plot(uniform_tvec,modelparam["dv"]*-1,  ls="-", c = "b" ,label="Normalized Observed dvv (flipped)")
        #ax[4].plot(uniform_tvec,modelparam["dv"],  ls="-", c = "b" ,label="Normalized Observed dvv")
        ax[4].plot(uniform_tvec, modelparam["soil"], label='soil-EWT',c='green')
        ax[3].plot(uniform_tvec, modelparam["soil"]/np.max(np.abs(modelparam["soil"])), label='Normalized soil-EWT',c='green')
        ...
        plt.tight_layout()
        plt.savefig(output_imgdir+"/MCMC_%s_%s_samplers.png"%(stnm, freqband), format="png", dpi=100)
        plt.close()
        plt.clf()  
        ...
        plt.savefig(output_imgdir+"/MCMC_%s_%s_%s_steps.png"%(stnm, freqband, modelparam["modelcase"]), format="png", dpi=100)
        plt.close()
        plt.clf()